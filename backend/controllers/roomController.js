...
exports.createRoom = async (req, res) => {
  try {
    ...
    const cleanedRoomNumber = String(roomNumber || "").trim();

    if (!cleanedRoomNumber || !seaterType || !monthlyFee) {
      return res.status(400).json({ message: "Room number, seater type and monthly fee are required" });
    }

    if (!/^[0-9]{1,4}$/.test(cleanedRoomNumber) || Number(cleanedRoomNumber) > 9999) {
      return res.status(400).json({ message: "Room number cannot exceed 9999." });
    }

    const exists = await Room.findOne({ roomNumber: cleanedRoomNumber });
