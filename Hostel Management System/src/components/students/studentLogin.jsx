...
const StudentLoginPage = () => {
  ...
  async function handleSubmit(e) {
    ...
    try {
      let data;
      if (tab === "login") {
        // calls POST /api/student/login → returns { token, student }
        data = await studentLogin(form.email, form.password);
      } else {
        // calls POST /api/student/register → returns { token, student }
        data = await studentRegister(form.name, form.email, form.password);
      }
  ...
  return (
    <div className="lp-root">
      {/* Left decorative panel — matches existing login style */}
