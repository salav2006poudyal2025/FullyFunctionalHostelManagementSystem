...
const { validateAddress, cleanText } = require("../utils/validation");
...
exports.createBooking = async (req, res) => {
  try {
    ...
    const cleanedPermanentAddress = String(permanentAddress || "").trim();
    const cleanedTemporaryAddress = String(temporaryAddress || "").trim();

    if (!cleanedFullName) {
      return res.status(400).json({ message: "Please enter your full name to proceed with the booking." });
    }

    const addressError = validateAddress(cleanedPermanentAddress, "Permanent address");
    if (addressError) {
      return res.status(400).json({ message: addressError });
    }

    if (cleanedTemporaryAddress) {
      const tempAddressError = validateAddress(cleanedTemporaryAddress, "Temporary address");
      if (tempAddressError) {
        return res.status(400).json({ message: tempAddressError });
      }
    }
    ...
    // Enforce strict capacity based on approved and pending reservations
    const pendingCount = await Booking.countDocuments({ room: roomId, status: "Pending" });
    if (room.occupiedSeats + pendingCount >= room.totalSeats) {
      return res.status(400).json({
        message: "Booking failed. This room has filled up since you loaded the page.",
      });
    }
    ...
    const booking = await Booking.create({
      ...
      phone,
      email: cleanedEmail,
      permanentAddress: cleanedPermanentAddress,
      temporaryAddress: cleanedTemporaryAddress,
      dob,
      educationStatus,
      room: roomId,
    });
